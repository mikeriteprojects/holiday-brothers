...
const OWNER_ACCOUNT_ID = 'ACC-0001'; // Herman's account — change requires a code edit, by design.
...
function getRows_(sheetName) {
  ...
  const data = sh.getRange(2, 1, lastRow - 1, hdrs.length).getValues();
  return data.map(function (row, i) {
    const obj = {};
    hdrs.forEach(function (h, j) { obj[h] = row[j]; });
    obj._row = i + 2; // 1-indexed sheet row, for in-place updates
    return obj;
  });
}
...
function calculatePrice_(size, sukkahType, speedTier) {
...
function actSubmitBooking_(p) {
  ...
  const price = calculatePrice_(answers.size, answers.sukkah_type, answers.speed_tier);
...
function actApproveVendor_(p) {
  ...
  if (referral && referral.referrer_account_id !== findRow_(SHEETS.VENDORS, 'vendor_id', p.vendor_id).account_id) {
